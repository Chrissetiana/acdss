...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en-US" lang="en-US" dir="ltr">
	<head>
		...
		<title>ACDSS | ADMIN | Archives</title>
									...
									<div class="art-postcontent art-postcontent-0 clearfix">
										...
										<?php
														...
														echo '<table class="art-article" cellpadding="2" cellspacing="10" style="border:0;width:100%;">';
															...
															while ($table_row = mysql_fetch_array($table_query))
															{
																echo '<tr>';
																	...
																	echo '<td bgcolor="#FFFFFF"><a href="admin_restore.php?table='.$table_row['location'].'&amp;id='.$table_row['id'].'&amp;action=restore"><img src="images/restore.png" title="Restore" width="20" height="20" /></a></td>';
																	echo '<td bgcolor="#FFFFFF">'.$table_row['uid'].'</td>';
																	echo '<td bgcolor="#FFFFFF">'.$table_row['fname'].' '.$table_row['mname'].' '.$table_row['lname'].'</td>';
											...
											else
											{
												...
												$table_query = mysql_query("SELECT id, uid, 'clients' AS location, fname, mname, lname FROM clients WHERE fname LIKE '%$keyword%' OR mname LIKE '%$keyword%' OR lname LIKE '%$keyword%' AND status='Inactive'")
																	 or die("This webpage is not available.<br />".mysql_error());
												if($table_query)
												{
													if (mysql_num_rows($table_query) != 0)
													{
														echo "<h3>".mysql_num_rows($table_query) ." record/s found. Sort in <a href='cswd_view.php?fname=".str_replace(" ", "+" ,$fname)."&order=asc&action=Search'>ascending</a> or <a href='admin_view.php?fname=".str_replace(" ", "+" ,$fname)."&order=desc&action=Search'>descending</a> order.</h3>";
														echo "<br />";
														echo '<table class="art-article" cellpadding="2" cellspacing="10" style="border:0;width:100%;">';
															echo '<tbody>';
																echo '<tr bgcolor="#666666">';
																...
																while ($table_row = mysql_fetch_array($table_query))
																{
																	echo '<tr>';
																		echo '<td bgcolor="#FFFFFF"><a href="?table='.$table_row['location'].'&amp;id='.$table_row['id'].'&amp;action=edit"><img src="images/edit.png" title="Edit" width="20" height="20" /></a></td>';
																		echo '<td bgcolor="#FFFFFF"><a href="admin_delete.php?table='.$table_row['location'].'&amp;id='.$table_row['id'].'&amp;action=delete"><img src="images/delete.png" title="Delete" width="20" height="20" /></a></td>';
																		echo '<td bgcolor="#FFFFFF">'.$table_row['uid'].'</td>';
																		echo '<td bgcolor="#FFFFFF">'.$table_row['fname'].' '.$table_row['mname'].' '.$table_row['lname'].'</td>';
																	echo '</tr>';
																}
